...
var arraySum = function(array) {
    ...
    if(array.length === 0) {
        ...
        for(var i = 0; i < array[0].length; i++) {
          innerSum += arraySum(array[0][i]);
        }
        return arraySum(array.slice(1,)) + innerSum;
    }
};
...
var isEven = function(n) {
    if(n === 0) {
        ...
        if(n > 1) {
            return isEven(n - 2*parseInt(n/2))
        } else {
            return isEven(-1*n)
        }
    }
...
var reverseArr = function(array) {
  if(array.length === 1) {
    ...
    var prefix = array.slice(0,array.length - 1);
    return [array[array.length - 1]].concat(reverseArr(prefix))
  }
};
...
var rMap = function(array, callback) {
  if(array.length === 1) {
    ...
    var prev = rMap(array.slice(0,array.length - 1), callback);
    prev.push(callback(array[array.length - 1]));
    return prev
  }
};
...
var replaceKeysInObj = function(obj, oldKey, newKey) {
  ...
  for(var innerKey in obj) {
    replaceKeysInObj(obj[innerKey], oldKey, newKey);
    if(innerKey === oldKey) {
...
var capitalizeWords = function(array) {
  if(typeof array === 'object') {
    for(var i = 0; i < array.length; i++) {
      array[i] = capitalizeWords(array[i]);
    }
    return array
  } else {
...
var capitalizeFirst = function(array) {
  if(typeof array === 'object') {
    for(var i = 0; i < array.length; i++) {
      array[i] = capitalizeFirst(array[i]);
    }
    return array
  } else {
...
var nestedEvenSum = function(obj) {
  if(Object.keys(obj) === 0 || typeof obj !== 'object') {
    return 0
  } else {
    var res = 0;
    for(var innerKey in obj) {
      res += nestedEvenSum(obj[innerKey])
      if(typeof obj[innerKey] === 'number' && obj[innerKey]%2 === 0) {
        res += obj[innerKey]
      }
    }
    return res
...
var letterTally = function(str, obj) {
  if(str.length === 0) {
    ...
    var res = obj === undefined ? {} : obj;
    if(str[str.length - 1] in res) {
      res[str[str.length - 1]] += 1;
    } else {
      res[str[str.length - 1]] = 1;
    }
    return letterTally(str.slice(0,str.length - 1), res)
  }
};
